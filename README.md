...
`parent` уже входят в доменную модель и будут использованы в отдельных кабинетах. Управление
участниками и приглашениями запланировано следующим этапом.
...
- bootstrap создаёт одного администратора; UI приглашений и смены организации пока отсутствует;
...
1. Управление участниками, приглашения и переключение организации.
2. Recording-ready webhook, transactional outbox и автоматический retry.
3. Отдельная транскрибация `faster-whisper` с сегментами и говорящими.
4. Интеграция `latex-for-everyone` для TEX/PDF/HTML.
5. Проверка и публикация материалов в кабинетах ученика и родителя.
6. Повторяющееся расписание, уведомления и iCal.
7. Наблюдаемость, audit log, backup/restore и политика retention.
